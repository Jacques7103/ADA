...
def schedule_page():
    ...
    lb = tk.Label(schedule_frame, text = "Schedule Page \nPage:3",font=('Bold',15))
    lb.pack()
    schedule_frame.pack(pady= 20)
    e = tk.Entry(schedule_frame, width =20)
    e.pack()
    myButton = tk.Button(schedule_frame, text="Enter Schedule",command= lambda:[texts.delete("1.0","end"), C1(e),add_date(word[0],scheduletexts), create_schedule(scheduletext, texts)])
    myButton.pack()

    texts = tk.Text(schedule_frame, width = 30, height = 6, font = ("Times New Roman",12))
    texts.pack()
    create_schedule(scheduletext, texts)

    my_button2 = tk.Button (schedule_frame, text = "Get Date",command=lambda: grab_date(my_label, my_label1, cal,scheduledata))
    my_button2.pack()
    
    cal = Calendar(schedule_frame, selectmode = "day", year= 2020, month = 5, day = 22)
    cal.pack()

    my_label = tk.Label(schedule_frame, text ="")
    my_label.pack(pady=5)
    my_label1 = tk.Label(schedule_frame, text ="")
    my_label1.pack()
